...
export function buildEmailHtml(payload) {
  ...
  const stageName = escapeHtml(payload.stage?.name || "");
  const phase = escapeHtml(payload.stage?.phase || "");
  const stageNum = payload.stage?.num ?? 0;
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background-color:#FDFBF7;border-radius:28px;overflow:hidden;box-shadow:0 24px 64px rgba(27,27,27,0.06);">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 12px;font-size:12px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;color:#C8A96B;">${phase} · Stage ${stageNum}</p>
